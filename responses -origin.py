...
def get_response(user_input: str):
    ...
    if validation_handler.check_is_num(split_user_input_list) and validation_handler.check_zipcode_datetime_provided_and_valid(split_user_input_list) != None:
        ...
        #if mile_range, zipcode and datetime are provided -> API
        #get the data from API and format response
        #update record
        if mile_range and input_zipcode and input_datetime:
            ...
            print("這裡是有mile_range and input_zipcode and input_datetime 後的res: ", res)
            return res

        #if zipcode and datetime are provided -> API
        #get the data from API and format response
        #update record
        else:
            nearby_dmv_offices_data =  dmv_api_handler.get_dmv_office_nearby_data_api(input_zipcode)
            #format response data AND update record
            res["response"] = format_response(split_user_input_list,nearby_dmv_offices_data)
            res["record"] = {"zipcode": input_zipcode,"datetime": input_datetime,"mile_range": mile_range}
            print("這裡是有input_zipcode and input_datetime 後的res: ", res)
            return res
    ...
    ## split_user_input_list includes string
        ...
        #check if date and zipcode are provided AND find zipcode and datetime
        ...
        for word in split_user_input_list:
            ...
            elif mile_range is None and input_datetime and input_zipcode:
